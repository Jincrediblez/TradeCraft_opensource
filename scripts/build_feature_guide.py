...
OUTPUT = ROOT / "output" / "pdf" / "TradeCraft_系统功能手册_zh-CN.pdf"
...
def build_story() -> list:
    story = [
        ...
        Spacer(1, 3 * mm),
        p("让主动交易者从成交记录走向证据、规则和持续改进", "cover_subtitle"),
        Spacer(1, 12 * mm),
        ColorBar(177 * mm, 6),
        Spacer(1, 11 * mm),
        box(
        ...
        PageBreak(),
        p("01  产品定位", "eyebrow"),
        p("TradeCraft 是一个交易复盘系统", "h1"),
        p(
            "它面向需要自己做判断、自己承担风险、并希望持续改进决策质量的主动交易者。"
            "TradeCraft 不负责告诉你明天买什么，而是帮助你准确回答：我做了什么、为什么做、结果如何、"
            "过程哪里失控、哪些优势有证据、下一次应该改变什么。",
            "body_large",
        ),
        Spacer(1, 3 * mm),
        pain_table(),
        Spacer(1, 7 * mm),
        box(
            "<b>核心价值：</b>把券商导出中的“成交事实”，转化为可以复核的交易回合、组合结果、行为模式和改进规则。"
            "长期价值不是多看一个图，而是减少同类错误的重复发生，并让真正有效的做法经得起样本和风险检验。"
        ),
        PageBreak(),
        p("02  使用者价值", "eyebrow"),
        p("主动交易者真正需要的不是更多数据，而是更好的复盘闭环", "h1"),
        p("一次完整的 TradeCraft 使用循环可以分成四层：", "body_large"),
        p("1. 看清事实", "h2"),
        bullet("导入真实成交和账户文件，确定交易日期、方向、数量、价格、佣金、持仓和账户结果。"),
        bullet("使用 FIFO 规则把零散 fill 还原成完整 BUY/SELL 交易回合。"),
        p("2. 重建决策", "h2"),
        bullet("把当时的 K 线、成交点、计划、无效条件和复盘笔记放在同一页面。"),
        bullet("减少事后诸葛亮和记忆重写，区分当时信息与后来结果。"),
        p("3. 识别模式", "h2"),
        bullet("从整个账本观察资金集中、交易频率、主题暴露、选股和退出行为。"),
        bullet("将账户结果、过程风险、近期行为和数据可信度分别判断。"),
        p("4. 形成改进", "h2"),
        bullet("下钻到证据，由用户确认或驳回系统发现。"),
        bullet("将一个高优先级问题转化为未来 20 个交易日的铁律，并持续验证。"),
        Spacer(1, 8 * mm),
        box(
            "<b>对主动交易者的意义：</b>TradeCraft 不承诺提高下一笔交易的收益，"
            "但它能提高复盘的可重复性、可追溯性和可执行性，让交易能力的改进不再只依赖感觉。",
            YELLOW,
        ),
        PageBreak(),
        p("03  数据与边界", "eyebrow"),
        ...
        p("真实数据流", "h2"),
        bullet("输入：IBKR .tlg、Activity Statement CSV、Transaction History CSV、MTM Summary CSV 和可选业绩工作簿。"),
        bullet("处理：解析、校验、FIFO 匹配、持仓对账、主题归因、七维评分和证据索引。"),
        bullet("输出：按年度隔离的本地状态、行情缓存、复盘页面、业绩和测评工作台。"),
        p("Demo 隔离", "h2"),
        bullet("每次 Demo reset 都生成新的 generation ID、随机交易、持仓、业绩、Watchlist、审计结果和离线 K 线。"),
        bullet("Demo 与真实数据互斥；真实文件上传前必须退出 Demo。"),
        bullet("Demo AI 根据合成快照离线生成，不调用 Kimi 或其他外部模型。"),
        p("联网边界", "h2"),
        bullet("复盘 Demo 使用离线合成 K 线。"),
        bullet("市场页面的 TradingView 和 Finviz 是明确可见的第三方联网组件，不接收本地账户和成交数据。"),
        box(
            "<b>开源安全习惯：</b>不要提交 .env、data/、cache/、logs/、数据库、券商导出、业绩工作簿或本机路径。"
    ...
    story.extend(
        [
            ...
            p("TradeCraft", "cover_title"),
            p("交易复盘系统", "cover_title"),
            p("把成交变成证据，把证据变成下一次行动。", "cover_subtitle"),
            Spacer(1, 14 * mm),
            ColorBar(177 * mm, 6),
            Spacer(1, 14 * mm),
            box(
...
def main():
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=17 * mm,
        bottomMargin=15 * mm,
        title="TradeCraft 交易复盘系统功能手册",
        author="TradeCraft contributors",
        subject="User-centered TradeCraft feature guide with synthetic Demo screenshots",
    )
